app [main!] { pf: platform "../../platform/main.roc", roc: "nightly-2026-09-26-d6267b4" }
...
number_operators = |a, b| {
...

	{
		...
		# `??` supplies a fallback when a `Try` is `Err`.
		default: [a].get(1) ?? 0,
...
question_postfix = |strings| {
	...
	# Postfix `?` is best when the existing error already has the right meaning.
	first_num = parse_number(first_str)?
...
main! = |_args| {
	...
	Stdout.line!(Str.inspect(question_postfix(["41"])))?
...
## Postfix propagation exposes the successfully decoded value to a test.
expect {
	answer = question_postfix(["41"])?
	answer == 42
}

## Missing input is mapped into a domain-specific error.
expect question_postfix([]) == Err(MissingNumber)

## Invalid numeric input retains the rejected text.
expect question_postfix(["forty-one"]) == Err(InvalidNumber("forty-one"))

## Pure helpers are tested at the top level rather than during application startup.
expect for_loop([1, 2, 3, 4, 5]) == 15
